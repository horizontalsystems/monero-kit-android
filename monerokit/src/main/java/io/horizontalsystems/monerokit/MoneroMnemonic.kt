...
/**
 * Monero legacy (Electrum-style) 25-word mnemonic: 24 data words that encode the spend key,
 * plus a checksum word repeated from among them. All words come from Monero's own 1626-word
 * English wordlist, which is disjoint from every BIP39 wordlist.
 */
object MoneroMnemonic {
    ...
    /**
     * Unique-prefix length of the English wordlist; the checksum is computed over word
     * prefixes, so it is a property of the wordlist, not a tunable.
     */
    const val PREFIX_LENGTH = 3

    private val wordSet: Set<String> by lazy { wordList.toSet() }

    fun isValidWord(word: String, partial: Boolean): Boolean = if (partial) {
        wordList.any { it.startsWith(word) }
    } else {
        word in wordSet
    }
    ...
    fun suggestions(prefix: String): List<String> {
        ...
        return wordList.filter { it.startsWith(prefix) }
    }

    /**
     * Selects the checksum word for [dataWords] (the first 24 words of a phrase): CRC32 over
     * the concatenated [PREFIX_LENGTH]-letter prefixes picks which data word is repeated last.
     */
    fun checksumWord(dataWords: List<String>): String {
        val crc = CRC32()
        val prefixes = dataWords.joinToString("") { it.take(PREFIX_LENGTH) }
        crc.update(prefixes.toByteArray(StandardCharsets.UTF_8))
        return dataWords[(crc.value % dataWords.size).toInt()]
    }
    ...
    fun validateChecksum(words: List<String>) {
        ...
        val expected = checksumWord(words.take(WORD_COUNT - 1))
        if (expected != words.last()) {
            throw InvalidChecksumException()
        }
    }

    class InvalidChecksumException : Exception("Invalid Monero mnemonic checksum")
